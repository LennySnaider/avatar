...
- `src/lib/billing/moduleFees.ts`: `chargeModuleFees(period = previousPeriodUtc())` → por `org_modules installed` × catálogo → `activity = UNIT_ACTIVITY[unit][slug](orgId)` (`bot.telegram = getTelegramBotActivity`, del sub-proyecto A, devolviendo cuándo estuvo activo cada bot — no un conteo) → días naturales tocados por unidad dentro del periodo (`unitDaysInPeriod`) → cuota = precio × (días-unidad / días del mes) → `chargeTokens` idempotente por `(módulo, YYYY-MM UTC)`. Cron `src/app/api/cron/module-fees/route.ts` (calco de `billing-holds-sweep`, `CRON_SECRET`), `vercel.json` `23 3 * * *` diario. **Se cobra POR VENCIDO**: el periodo por defecto es el mes ANTERIOR, porque el prorrateo sólo se puede calcular sobre un mes ya cerrado — un bot conectado el 28 de septiembre paga sólo esos 3 días, no el mes entero. La idempotencia sigue siendo una vez al mes (el PRIMER pase que ve actividad en el periodo asienta el cargo, sea cual sea el día); correr el cron a diario no es el prorrateo, es tolerancia a que el pase del día 1 falle.
...
// A implementa getTelegramBotActivity(orgId): Promise<UnitActivity[]> en src/lib/telegram/bots.ts
//   (una entrada por bot: activeFrom = cuándo se conectó, activeUntil = cuándo se desconectó o
//   null si sigue activo) y la registra con registerUnitActivity('telegram', getTelegramBotActivity)
//   (B). Ya no es un conteo: B prorratea por días y necesita saber desde/hasta cuándo estuvo
//   activo cada bot, no cuántos hay.
```
...
- **`avatar_telegram_settings`** (plan previo +): `bot_id bigint unique`, `direct_enabled`, `business_connection_id`, `business_user_id`, `business_user_chat_id`, `business_rights jsonb`, `business_is_enabled`, `business_connected_at/disconnected_at`, `welcome_enabled`, `welcome_message`, `takeover_pause_minutes default 30`, `ai_offers_enabled`, `thanks_after_purchase_enabled default true`, `thanks_message text` (null = lo genera la IA en personaje), `last_update_at`, `last_error`. `getTelegramBotActivity(orgId): Promise<UnitActivity[]>` (contrato B, `registerUnitActivity`) = una fila por bot con `activeFrom` (cuándo se conectó) y `activeUntil` (cuándo se desconectó, o `null` si sigue activo) — ya no un `count(*)`: B prorratea por días y necesita saber desde/hasta cuándo estuvo activo cada uno, no cuántos hay.
