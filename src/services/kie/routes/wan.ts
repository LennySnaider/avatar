    ...
    if (ctx.referenceImage) {
        try {
            ...
            if (cloneRef) {
                ...
                const faceIdentityClause = ` Swap ONLY the FACE: use the SECOND attached image's face — exact features, bone structure and likeness, NEVER the first image's face.${faceFidelityClause} Blend the swap invisibly: relight the face to the scene's own light direction and colour, with the same grain, sharpness and skin texture as the rest of the photo — no pasted-on look, the head at natural size for the body.`
