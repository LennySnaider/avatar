...
// Individual toast item with CSS transition
const ToastItem = ({
    item,
    variants,
    toastProps,
    ...
    item: Message
    variants: { initial: Record<string, any>; animate: Record<string, any>; exit: Record<string, any> }
    toastProps: Record<string, any>
    toastRef: Ref<ToastWrapperInstance>
    onClose: () => void
}) => {
    ...
    const currentState = !isAnimated ? variants.initial : (item.visible ? variants.animate : variants.exit)
...
const useMessages = (msgKey: string) => {
    ...
    const push = useCallback(
        (message: NodeProps) => {
            ...
            setMessages([...messages, { key, visible: true, node: message, mounted: true }])
            return key
        },
        [messages, msgKey],
    )
