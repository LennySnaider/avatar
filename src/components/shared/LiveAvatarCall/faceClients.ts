...
export async function createFaceClient(config: FaceClientConfig, els: FaceElements): Promise<FaceClient> {
    switch (config.provider) {
        ...
        case 'simli':
            return createSimli(config.sessionToken, els)
        case 'liveavatar':
...
async function createSimli(sessionToken: string, els: FaceElements): Promise<FaceClient> {
    const { SimliClient, LogLevel } = await import('simli-client')
    // Transporte 'livekit': no necesita servidores ICE propios (el 'p2p'
    // exigiría pedirlos con la API key, que no sale del servidor).
    const client = new SimliClient(sessionToken, els.video, els.audio, null, LogLevel.ERROR, 'livekit')
    return {
        async start() {
            await client.start()
        },
        ...
        endTurn() {
            // Simli no necesita marca de fin: con `handleSilence` rellena solo.
        },
        interrupt() {
            client.ClearBuffer()
        },
