    ...
    if (m.glutesLevel && SHEET_GLUTES_PHRASE[m.glutesLevel]) {
        parts.push(
            m.glutesLevel >= 6 && (m.hips ?? 0) < 130
                ? 'MASSIVE exaggerated BBL-style glutes far beyond natural anatomy — ALL of the volume projects straight BACKWARD as an extreme shelf-like curve seen from the side and back, while her hip width stays true to her measured hips from the front AND from behind — the mass is depth, not sideways spread'
...
export function buildBodySheetPrompt(
    ...
    // El vello púbico también lo FIJA la hoja nude: si no, cambia de una
        ...
        isXXL
            ? // El detalle por-vista del XXL también debe ser coherente con
              ...
              (m.hips ?? 0) >= 130
                ...
                : 'IMPORTANT: her body is DRAMATICALLY different from the reference — RESHAPE it completely to the spec above; the reference is ONLY for the poses, views, framing and background, NEVER for the body proportions. The SAME proportions in EVERY view: FRONT — her waist exactly as the measurements specify and hip width true to her measured hips, inner thighs touching; THREE-QUARTER and SIDE — EXTREME shelf-like glute projection straight backward with a deep lower-back curve (this is where her glutes look most dramatic); BACK — MASSIVE glutes whose size reads as DEPTH and roundness, never as width: her hip line from behind is exactly as narrow as in the front view, and the glutes do not spread sideways past it.'
