...
export function buildLeanIdentityPrompt(
    ...
    if (refRoles[0] === 'scene') {
        ...
        if (faceIsImage) {
            ...
            const opening = `Image 1 is the photo to recreate: keep its EXACT pose, outfit, body shape, framing, lighting and setting — but the FACE of the person in Image 1 is the WRONG identity and must be entirely replaced. Take the face and head identity ENTIRELY from Image 2. Completely DISCARD and ERASE Image 1's original face: do NOT keep, blend, average, mix, merge or interpolate ANY of its facial features (eyes, eyebrows, nose, lips, cheekbones, jaw, or face shape). The output face must be 100% the person in Image 2 and 0% the person originally in Image 1 — recognizably the SAME individual as Image 2, never a hybrid of the two. Copy Image 2's defining features EXACTLY: the exact eye shape AND eye color (e.g. green, hazel, blue — match the precise hue), the eyebrow shape, the skin texture, and the overall face shape. If Image 1's original face had different eyes, brows, marks or face shape, OVERRIDE them with Image 2's. Replace ONLY the face/head; keep everything else in Image 1 identical.`
            note = `${opening} ${tail}`
        } else {
