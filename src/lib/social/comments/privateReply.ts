...
export async function maybeSendCommentDm(input: MaybeSendCommentDmInput): Promise<MaybeSendCommentDmResult> {
    ...
    // `comment_id` UNIQUE en la BD es el cinturón para la carrera del insert
    // más abajo, esto es la comprobación previa para no ni intentarlo.
    ...
    try {
        ...
        if (error) {
            if (/duplicate key/i.test(error.message)) {
                console.warn('[social] DM ya registrado por otra ejecución concurrente', {
                    avatarId: chat.avatar_id,
                    commentId: input.commentId,
                })
                return 'skipped'
        ...
        if (insertError) {
            if (/duplicate key/i.test(insertError.message)) return 'skipped'
