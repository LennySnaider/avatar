...
-- en tiempo real — cara en video (proveedor intercambiable: Anam por defecto,
-- Simli como opción barata), voz clonada (MiniMax) y la persona del agente —
...
create table if not exists avatar_live_settings (
    ...
    -- src/lib/live/face/.
    face_provider text not null default 'anam'
        check (face_provider in ('anam', 'liveavatar', 'simli')),
    face_id text,
